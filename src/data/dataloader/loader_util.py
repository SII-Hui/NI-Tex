...
class BaseDataset(Dataset):
    ...
    def recolor_image_with_background(self, pil_img, new_color, background_color=(0,0,0), image_size=None):
        """
        将纯色前景替换为指定颜色，同时对透明背景使用给定背景色
        
        Args:
            pil_img: 图片路径或 PIL.Image 对象
            new_color: (R, G, B) 前景替换颜色
            background_color: (R, G, B) 背景填充颜色
            image_size: 输出大小 (可选)
        Returns:
            image_tensor: 替换后的 RGB Tensor, shape (3,H,W)
            alpha_tensor: Alpha Tensor, shape (3,H,W)
        """
        if image_size is None:
            image_size = self.image_size
        ...
        data = np.array(pil_img, dtype=np.float32) / 255.0
        alpha = data[:, :, 3:4]  # shape (H,W,1)

        new_rgb = np.zeros_like(data[:, :, :3])
        new_rgb[:, :, 0] = new_color[0] / 255.0
        new_rgb[:, :, 1] = new_color[1] / 255.0
        new_rgb[:, :, 2] = new_color[2] / 255.0

        bg = background_color

        image = new_rgb * alpha + bg * (1 - alpha)

        image_tensor = torch.from_numpy(image).permute(2,0,1).contiguous().float()
        alpha_tensor = torch.from_numpy(np.repeat(alpha, 3, axis=2)).permute(2,0,1).contiguous().float()
