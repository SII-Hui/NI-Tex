...
def _apply_background_mask(content: torch.Tensor, visible_mask: torch.Tensor, 
    ...
    bg_tensor = torch.tensor(bg_color, dtype=torch.float32, device=device) # bg_tensor: [1,1,1]
    return content * visible_mask + bg_tensor * (1 - visible_mask) # content.shape: torch.Size([1, 2048, 2048, 3]); visible_mask.shape: torch.Size([1, 2048, 2048, 1])
'''

def _apply_background_mask(content: torch.Tensor, visible_mask: torch.Tensor, 
                          bg_color: List[float], device: str) -> torch.Tensor:
    """
    Apply transparent background to masked regions.

    Args:
        content: tensor of shape [N, H, W, 3], RGB values in [0,1].
        visible_mask: tensor of shape [N, H, W, 1], values 0/1.
        device: device string

    Returns:
        Tensor of shape [N, H, W, 4], RGBA
    """
    alpha = visible_mask  # alpha=1 where visible, 0 else
    # For RGB, keep content where visible, otherwise 0 (or any color since transparent)
    rgb = content * visible_mask  # invisible areas blacked out, with alpha=0 means transparent
    rgba = torch.cat([rgb, alpha], dim=-1)  # concat alpha channel
...
class MeshRender:
    def __init__(
        ...
        self.bake_angle_thres = 125 # # revised
        ...
        if camera_type == "orth":
            self.set_orth_scale(orth_scale)
    ...
    def back_project(self, image, elev, azim, camera_distance=None, center=None, method=None):
        ...
        # Image.fromarray(((visible_mask.squeeze(-1)-visible_mask.min())/(visible_mask.max()-visible_mask.min())*255).byte().cpu().numpy(), 'L').save('output.png')
        ...
        if self.bake_unreliable_kernel_size > 0:
            ...
            # visible_mask = torch.sigmoid((visible_mask - 0.5) * 10) # revised
    ...
    def uv_inpaint(self, texture, mask, vertex_inpaint=True, method="NS", return_float=False):
        ...
        if isinstance(texture, torch.Tensor):
            texture_np = texture.cpu().numpy()*255 # revised
        ...
        if vertex_inpaint:
            ...
            texture_np_1, mask = meshVerticeInpaint(texture_np, mask, vtx_pos, vtx_uv, pos_idx, uv_idx) # revised

        if method == "NS":
            texture_np = cv2.inpaint((texture_np[:,:,:3]).astype(np.uint8), 255 - mask, 3, cv2.INPAINT_NS) # revised
