...
if __name__ == "__main__":
    ...
    if getattr(config, "init_unet_from", None):
        ...
        model_unet.load_state_dict(sd, strict=False)
    ...
    if getattr(config, "init_vae_from", None):
        ...
        model.pipeline.vae.load_state_dict(sd_vae, strict=False)
    ...
    if noise_in_channels is not None:
        with torch.no_grad():
            new_conv_in = torch.nn.Conv2d(
                ...
                model_unet.conv_in.out_channels, # 320
                model_unet.conv_in.kernel_size, # (3,3)
                model_unet.conv_in.stride, # (1,1)
    ...
    ckpt_files = glob.glob(os.path.join(ckptdir, "step=*.ckpt"))
    patt = re.compile(r"step=(\d{6})_(\d{8})\.ckpt$")
    ...
    real_ckpt_path = None
    if latest_ckpt and (m := patt.search(os.path.basename(latest_ckpt))):
        last_round = int(m.group(1))          # 000010 -> 10
        new_round  = last_round + 1           # -> 11
        real_ckpt_path = latest_ckpt          # 用于 resume
    else:
        new_round = 1                         # 第一次训练
        cnet_ckpt_path = None
    filename_tpl = f"step={new_round:06}_{{step:08}}"


    if real_ckpt_path is not None:
        sds = torch.load(real_ckpt_path, map_location="cpu")["state_dict"]
        ...
        rank_zero_print(f"Loaded {real_ckpt_path} ...")
    elif getattr(config, "resume_from", None):
