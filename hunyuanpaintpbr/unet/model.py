...
class HunyuanPaint(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # init modules
        pipeline = DiffusionPipeline.from_pretrained(**stable_diffusion_config) # 2.5d
        ...
        self.vision_enc = CLIPVisionModelWithProjection.from_pretrained(
    ...
    def prepare_batch_data(self, batch):
        ...
        cond_imgs = images_cond[:, 0:1, ...] # 选择第一个
        ...
        return cond_imgs, target_imgs, images_normal, images_position, batch["images_ref_paths"][0] # 选择第一个

    @torch.no_grad()
    def forward_text_encoder(self, prompts):
        device = next(self.pipeline.vae.parameters()).device
        text_embeds_3dtopia = self.pipeline.encode_prompt(prompts[0], device, 1, False)[0] # torch.Size([1, 77, 1024])
        text_embeds_cap3d = self.pipeline.encode_prompt(prompts[1], device, 1, False)[0]
        return text_embeds_3dtopia, text_embeds_cap3d
    ...
    def training_step(self, batch, batch_idx):
        ...
        # print(f"[rank {self.global_rank}] starts training!")

        cond_imgs, target_imgs, normal_imgs, position_imgs, ref_image_paths = self.prepare_batch_data(batch)

        mr_training = not any('image_prompt' in s for s in ref_image_paths[0])


        # B, N_ref = cond_imgs.shape[:2]
        B = normal_imgs[0].shape[0]
        ...
        # 1) 前处理
        pixel_values = self.processor(cond_imgs[0], return_tensors="pt").pixel_values  # shape (1, 3, 224, 224), [-1,1]
        # 2) 编码
        with torch.no_grad():
            embeds = self.vision_enc(pixel_values.to("cuda")).image_embeds  # shape (1, 1024)
            text_embeds = embeds.unsqueeze(1).repeat(1,77,1)
        '''
        ...
        if self.unet.use_dino:
            dino_hidden_states = self.dino_v2(cond_imgs[:, :1, ...]) # torch.Size([1, 257, 1536])
            # dino_hidden_states_another = self.dino_v2(cond_imgs_another[:, :1, ...]) # torch.Size([1, 257, 1536])
        ...
        if position_imgs is not None:
            ...
            cached_condition["embeds_position"] = position_embeds  #! B, N, C, H, W # torch.Size([1, 6, 4, 64, 64])
            cached_condition["position_maps"] = position_imgs[0]  #! B, N, C, H, W # torch.Size([1, 6, 3, 512, 512])
        ...
        if self.train_scheduler.config.prediction_type == "v_prediction":
            ...
            if mr_training:
                ...
                albedo_loss = albedo_loss_1
                mr_loss = mr_loss_1
                sum_loss = albedo_loss + mr_loss
            else:
                albedo_loss_1, _ = self.compute_loss(v_pred_albedo, v_target_albedo)
                albedo_loss = albedo_loss_1 * 2
                sum_loss = albedo_loss

            log_loss_dict = {}
            log_loss_dict.update({f"train/albedo_loss": albedo_loss})
            # log_loss_dict.update({f"train/mr_loss": mr_loss})
            log_loss_dict.update({f"train/sum_loss": sum_loss})
    ...
    def on_validation_epoch_start(self):
        free_mem = torch.cuda.memory_reserved(dist.get_rank()) / 1024**3
        print(f"[rank {dist.get_rank()}] 可用GPU内存: {free_mem:.2f}GB")
        torch.distributed.all_reduce(torch.tensor([1.0]).cuda())  # 测试通信

        # 确保所有进程都到达验证开始点
        print(f"[rank {dist.get_rank()}] 等待验证同步...")
        torch.distributed.barrier()
        print(f"[rank {dist.get_rank()}] 验证同步完成")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        print(f"[rank {self.global_rank}] starts validation!")
        
        # if batch is None:         
        #     return None

        cond_imgs_tensor, target_imgs, normal_imgs, position_imgs, ref_image_paths = self.prepare_batch_data(batch)
        ...
        if 'png' not in ref_image_paths[0]: 
            output_path = self.logdir + '/images_val/images/' + batch['name'][0].split('/')[-1]
            os.makedirs(output_path, exist_ok=True)
        else:
            output_path = self.logdir + '/images_val/images/' + batch['name'][0].split('/')[-1] + '_' + os.path.splitext(os.path.basename(ref_image_paths[0]))[0].split('_')[0]
            os.makedirs(output_path, exist_ok=True)

        output_path_file = os.path.join(output_path, "image_prompt.png")
        shutil.copy(ref_image_paths[0], output_path_file)
        ...
        for idx in range(len(image_pils)):
            ...
            ) # torch.Size([1, 2, 10, 3, 512, 512])

            ### save images
            img_temp = image.squeeze(0).permute(1,0,2,3,4)  # (10, 2, 3, H, W)
            for group_idx in range(img_temp.shape[0]):        # 0 ~ 9
                for view_idx in range(img_temp.shape[1]):     # 0 ~ 1
                    img = img_temp[group_idx, view_idx]       # (3, H, W)
                    # 如果你的像素范围已是 0~1，可直接乘 255；若是 -1~1 需先转换
                    img = torch.clamp(img, 0, 1) * 255
                    img = img.byte().cpu()
                    # (C, H, W) -> (H, W, C)  再转 PIL Image
                    img_pil = Image.fromarray(img.permute(1, 2, 0).numpy())
                    file_name = f"{group_idx+1}_{view_idx+1}.png"
                    img_pil.save(os.path.join(output_path, file_name))
            txt_dir = output_path
            # prompts = batch['text_prompt']
            # if isinstance(prompts, str):
            #     with open(os.path.join(txt_dir, 'prompt.txt'), 'w', encoding='utf-8') as f:
            #         f.write(prompts)
            # else:
            #     for i, p in enumerate(prompts):
            #         with open(os.path.join(txt_dir, f'text_{i+1}.txt'), 'w', encoding='utf-8') as f:
            #             f.write(str(p))
            ### save images


            image = torch.cat((torch.ones_like(image[:, :, :1, ...]) * 0.5, image), dim=2) # torch.Size([1, 2, 10, 3, 512, 512])
        ...
        print(f"[rank {self.global_rank}] finishes validation!")
